...
%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
Here are a few examples illustrating how ubiquitous matrices are in mathematics
\vfill
...
Let $\bx(t)=(x_1(t),\ldots,x_n(t))\in \IR^n$ be a \defword{state variable} and $t\in \IN$ be an \defword{independent variable}, typically thought of as \defword{time}. Let $A\in \mathcal{M}_n(\IR)$. ($\M_n(\IR)$ is also sometimes denoted $\IR^{n\times n}$)
\vfill
...
\end{align}	
\end{subequations}
where $\bx_0$ is called the \defword{initial condition} (IC)
\vfill
...
A linear map corresponding to a matrix with all eigenvalues of modulus less than 1 is a \defword{linear contraction}, with the origin a \defword{linear sink} or \defword{attracting fixed point}
\vfill
If all eigenvalues have modulus larger than 1, then the map induced by $A$ is a \defword{linear expansion}, and the origin is a \defword{linear source} or \defword{repelling fixed point}
\vfill
The map $A\bx$ is a \defword{hyperbolic linear map} if all eigenvalues of $A$ have modulus different of 1
\end{frame}
...
\end{equation}
where $\bx(t)\in \IR^n$ is a \emph{function} and $f:\IR^n \to \IR^n$ is a \defword{vector field}
\vfill
An \defword{initial value problem} (IVP) is the consideration of \eqref{eq:nonlinear_ODE} together with an initial condition $\bx(t_0)=\bx_0\in\IR^n$
\vfill
...
\end{definition}
The term \defword{flow} is used since $e^{At}$ describes the motion of points $\bx_0\in\mathbb{R}^n$ along trajectories of \eqref{eq:Lin_EDO_cc_hom}
%%
\vfill
%%
\begin{definition}
...
\bx'=f(\bx),
\end{equation}
where $\bx\in\IR^n$ and $f:\IR^n\to\IR^n$ is $C^1$. A standard step when studying \eqref{sys:ODE} \defword{qualitatively} is to seek \defword{equilibria} of \eqref{sys:ODE}, \ie, points $\bx^\star\in\IR^n$ such that
...
\begin{definition}
An equilibrium point $\bx^\star$ is \defword{hyperbolic} if the Jacobian matrix $Df$ of \eqref{sys:ODE} evaluated at $\bx^\star$, denoted $Df(\bx^\star)$, has no eigenvalues with zero real part, \ie, $Df(\bx^\star)$ is invertible
\end{definition}
\vfill
...
\frac{d}{dt}\bx=A(t)\bx+\bB(t),\tag{LNH}
\end{equation}
where $A(t)\in\mathcal{M}_n(\IR)$ with continuous entries, $\bB(t)\in\IR^n$ with real valued, continuous coefficients, and $\bx\in\IR^n$
\vskip0.5cm
The associated IVP takes the form 
\begin{equation}\label{sys:IVP_linear_general}
\begin{aligned}
\frac{d}{dt}\bx &= A(t)\bx+\bB(t) \\
\bx(t_0)&=\bx_0
\end{aligned}
\end{equation}
\end{definition}
...
\frame{\frametitle{Types of linear systems}
\begin{itemize}
\item $\bx'=A(t)\bx+\bB(t)$ is linear nonautonomous ($A(t)$ depends on $t$) nonhomogeneous (also called \emph{affine} system)
\vfill
\item $\bx'=A(t)\bx$ is linear nonautonomous homogeneous
\vfill
\item $\bx'=A\bx+\bB$, that is, $A(t)\equiv A$ and $\bB(t)\equiv \bB$, is linear autonomous nonhomogeneous (or affine autonomous)
\vfill
\item $\bx'=A\bx$ is linear autonomous homogeneous
\end{itemize}
}
...
Solutions to \eqref{sys:IVP_linear_general} exist and are unique on the whole interval over which $A$ and $\bB$ are continuous
\vskip0.5cm
In particular, if $A,\bB$ are constant, then solutions exist on $\IR$
\end{theorem}
}
...
\frac{d}{dt}\bx=A\bx+\bB\tag{A}
\end{equation}
and the associated homogeneous autonomous system
\begin{equation}\label{sys:lin_auton}
\frac{d}{dt}\bx=A\bx\tag{L}
\end{equation}
}

\frame{\frametitle{Exponential of a matrix}
\begin{definition}[Matrix exponential]
Let $A\in\M_n(\IF)$ with $\IF=\IR$ or $\IC$. 
The \defword{matrix exponential} of $A$, denoted $e^{At}$, is a matrix in $\M_n(\IF)$, defined by
\[
...
\begin{frame}\frametitle{Properties of the matrix exponential}
\begin{itemize}
\item $e^{At_1}e^{At_2}=e^{A(t_1+t_2)}$ for all $t_1,t_2\in\IR$
\vfill
\item $Ae^{At}=e^{At}A$ for all $t\in\IR$
\vfill
\item $(e^{At})^{-1}=e^{-At}$ for all $t\in\IR$
\vfill
\item The unique solution $\phi$ of \eqref{sys:lin_auton} with $\phi(t_0)=\bx_0$ is given by
\[
\phi(t)=e^{A(t-t_0)}\bx_0
\]
\end{itemize}
\end{frame}
...
\frame{
\begin{theorem}
For all $(t_0,\bx_0)\in\IR\times\IR^n$, there is a unique solution $\bx(t)$ to \eqref{IVP:lin_auton} defined for all $t\in\IR$. Each coordinate function of $\bx(t)$ is a linear combination of functions of the form
\[
...
Let $A\in\M_r(\IR)$. Suppose $\lambda$ is an eigenvalue of $A$ with multiplicity $m\leq n$. Then, for $k=1,\ldots,m$, any nonzero solution $\bv$ of
\[
(A-\lambda\II)^k\bv=\b0
\]
...
\frame{
\begin{theorem}
Under conditions of the Jordan normal form Theorem, the linear system $\bx'=A\bx$ with initial condition $\bx(0)=\bx_0$, has solution
\[
\bx(t)=P\diag\left(e^{\lambda_j t}\right)P^{-1}\left(\II+Nt+\cdots\frac{t^k}{k!}N^k\right) \bx_0
\]
\end{theorem}
\vfill
\begin{theorem}[Case of an eigenvalue of multiplicity $n$]
Suppose that $\lambda$ is an eigenvalue of multiplicity $n$ of $A\in\M_n(\IR)$. Then $S=\diag(\lambda)$, and the solution of $\bx'=A\bx$ with initial value $\bx_0$ is given by
\[
\bx(t)=e^{\lambda t}\left(\II+Nt+\cdots\frac{t^k}{k!}N^k\right) \bx_0
\]
\end{theorem}
In the simplified case, we do not need the basis of generalized eigenvectors (i.e., $P$)
}
 ...
 \end{align}
 \end{subequations}
 where $B:\IR\to\IR^n$ a smooth function on $\IR$ and $e^{A(t-t_0)}$ is the matrix exponential associated to the homogeneous system $\bx'=A\bx$
 \vskip0.5cm
 Then the solution $\phi$ to \eqref{sys:lin_nonlin} is given by
 \begin{equation}
 \phi(t)=e^{A(t-t_0)}\bx_0 + \int_{t_0}^t e^{A(t-s)}B(s)ds
 \end{equation}
 \end{theorem}
 }
...
\vfill
\begin{theorem}
Let $\G(M)$ be the digraph induced by $M$. We have $0\leq M\in\M_n$ irreducible $\iff$ $\G(M)$ strongly connected
\end{theorem}
\end{frame}
