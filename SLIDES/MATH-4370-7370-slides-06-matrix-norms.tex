...
\vfill
\begin{definition}
Let $x\in \IF^n$. Denote $| x|= [|x_i|]$ ($|\cdot |$ entry-wise), and write that $|x|\leq |y |$ if $|x_i|\leq |y_i|$ for all $i= 1, \dots, n$. Then $\|\cdot\|$ is 
\begin{enumerate}
    \item \defword{monotone} if $|\bx|\leq |\by| \implies \|\bx\| \leq \|\by\|$ for all $\bx,\by\in\IF^n$
    \item \defword{absolute} if $\left \| |\bx| \right\| $ for all $\bx\in\IF^n$
\end{enumerate}
\end{definition}
\end{frame}
...
\begin{enumerate}
    ...
    \item $\|\cdot\|$ absolute if and only if $\|\cdot\|$ monotone
\end{enumerate}
\end{theorem}
\end{frame}
...
\begin{enumerate}
    ...
    \item $\tripbar A \tripbar = \max\limits_{\|\bx\|= \|\by\|^D=1} |\by^* A\bx|$
\end{enumerate}
\end{theorem}
\end{frame}
...
\begin{frame}
	\begin{theorem}
		Let $A\in\M_n$ and $\varepsilon>0$ be given.
		Then there exists a matrix norm $\tripbar\,\cdot\,\tripbar$ such that
		\[
			\rho(A)\leq\tripbar A\tripbar\leq \rho(A)+\varepsilon
		\]
	\end{theorem}
\end{frame}
