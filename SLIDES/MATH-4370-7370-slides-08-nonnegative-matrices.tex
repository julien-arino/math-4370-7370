...
A matrix $A\in \M_{mn}(\IR)$ is a \defword{nonnegative matrix} if $a_{ij}\geq 0$ for all $i=1,\ldots,m$ and $j=1,\ldots,n$. We write $A\geq\b0$ or, often here, $\b0\leq A\in\M_n(\IR)$
\vskip0.5cm
$A$ is a \defword{positive matrix} if $a_{ij}>0$ for all $i=1,\ldots,m$ and $j=1,\ldots,n$. We write $A>\b0$ or, often here, $\b0<A\in\M_n(\IR)$
\end{definition}
\vfill
Unless otherwise specified, all matrices here have real entries, so read $\M_n$ as meaning $\M_n(\IR)$
\end{frame}

\begin{frame}{Another (nicer in my view) notation}
\begin{remark}
In other references, you will see
\begin{itemize}
    \item $A\geq\b0 \iff a_{ij}\geq 0$
    \item $A>\b0 \iff A\geq\b0$ and $A\neq\b0$ (i.e., there exists $(i,j)$, $a_{ij}>0$)  \hfill [positive]
    \item $A\gg \b0 \iff a_{ij}>0$ for all $i,j$ \hfill [strongly positive]
\end{itemize}
\end{remark}
\vfill
I favour this notation over the one used in these notes, but since the former is more common in matrix theory, I use the notation of Definition~\ref{def:nonnegative_matrix} here
\end{frame}
...
\begin{itemize}
    \item $A\geq B \iff A-B\geq \b0$
    \item $A>B \iff A-B >\b0$
\end{itemize}
\vfill
The same is used for vectors $\bx,\by\in\IR^n$: $\bx\geq\by$ and $\bx>\by$ if, respectively, $\bx-\by\geq\b0$ and $\bx-\by>\b0$
\vfill
Note that the order is only partial: if $A\geq \b0$ and $B\geq \b0$, for instance, it is not necessarily possible to decide on the ordering of $A$ and $B$ with respect to one another
\end{frame}
...
If $A>\b0$ and $B\geq \b0$, $B\neq \b0$, then $AB\geq \b0$ and $AB\neq \b0$
\end{theorem}
\vfill
\begin{corollary}\label{coro:nonnegativity_preserves_orientation}
Let $\bx,\by\in\IR^n$ be such that $\bx\geq\by$ and $\b0\leq A\in \M_{mn}$. Then $A\bx\geq A\by$. 
Assume additionally that $\bx\geq\by$, $\bx\neq\by$ and $A>\b0$. Then $A\bx>A\by$
\end{corollary}
\end{frame}
...
\begin{frame}
\begin{definition}
Let $P,Q\in \M_{mn}(\IF)$. $P$ and $Q$ have the same \defword{zero-nonzero structure} if for all $i,j$, $p_{ij}\neq 0 \iff q_{ij}\neq 0$
\end{definition}
\vfill
...
\begin{itemize}
	...
	\item $1+0=0+1=1$
	\item $1+1=1$
	\item $0\cdot 0= 0\cdot 1=1\cdot 0= 0$
	\item $1\cdot 1=1$
\end{itemize}
\end{definition}
\begin{definition} 
...
Let $\b0\leq A\in \M_n$ be \textbf{irreducible}. Then the spectral radius $\rho(A)= \max \{ |\lambda|, \lambda\in \sigma(A)\}$ is an eigenvalue of $A$. It is simple (has algebraic multiplicity 1), positive and is associated with a positive eigenvector. Furthermore, there is no nonnegative eigenvector associated to any other eigenvalue of $A$
\end{theorem}
\vfill
\begin{remark}
...
Let $\b0<A\in\M_n$. Then $\rho(A)$ is a positive eigenvalue of $A$ and there is only one linearly independent eigenvector associated to $\rho(A)$, which can be taken to be positive
\end{lemma}
\vfill
\begin{lemma}\label{lemma:prep_Perron}
Let $\alpha_1,\ldots,\alpha_n\in\IR_{+}\setminus\{0\}$ and $v_1,\ldots,v_n\in\IC$. Then
...
\begin{definition}
Let $\b0\leq A\in\M_n$.
We say that $A$ is \defword{primitive} with \defword{primitivity index} $k\in \IN_{+}\setminus\{0\}$ if there exists $k\in \IN_{+}\setminus\{0\}$ such that 
\[
A^k>\b0
\]
...
\begin{frame}
\begin{theorem}
A sufficient condition for primitivity of $\b0\leq A\in\M_n$ is irreducibility with at least one positive diagonal entry
\end{theorem}
\vfill
...
\vfill
\begin{theorem}
Let $\b0\leq A\in \M_n$. If $A$ is primitive, then $A^k>\b0$ for some $0<k\leq (n-1)n^n$
\end{theorem}
\end{frame}
...
\begin{frame}
\begin{theorem}
Let $\b0\leq A\in\M_n$ be primitive. Suppose the shortest simple directed cycle in $G(A)$ has length $s$, then the primitivity index of $A$ is $\leq n+s(n-1)$
\end{theorem}
\vfill
\begin{theorem}[Wielandt's Theorem]
Let $\b0\leq A\in \M_n$. $A$ is primitive if and only if $A^{n^2-2n+2}>\b0$
\end{theorem}
\vfill
\begin{theorem}
Let $\b0\leq A\in \M_n$ be irreducible. Suppose that $A$ has $d$ positive entries on the diagonal. Then the primitivity index of $A$ is $\leq 2n-d-1$
\end{theorem}
\end{frame}
...
\begin{frame}
\begin{theorem}
Let $\b0\leq A\in\M_n$. Then there exists $\b0\neq \bv\geq \b0$ such that $A\bv= \rho(A)\bv$
\end{theorem}
\end{frame}
...
The classification in the result below follows the presentation in \cite{Caswell2001}.
\begin{theorem}\label{th:PF_allCases}
Let $\b0\leq A\in\M_n$. Denote $\lambda_P$ the Perron root of $A$, i.e., $\lambda_P=\rho(A)$, $\bv_P$ and $\bw_P$ the corresponding right and left Perron vectors of $A$, respectively
\vskip0.5cm
...
\vskip0.5cm
Conclusions of the Perron-Frobenius Theorem can be summarised as in the graph on the next slide
\end{theorem}
\end{frame}
...
\begin{frame}
\begin{theorem}
Let $\b0\leq A\in \M_n$ be irreducible and $h\in \IN_{+}$. TFAE:
\begin{enumerate}
    ...
    \item $h$ is the maximal positive integer $k$ such that $\sigma(e^{2\pi i /k}A)= \sigma(A)$
\end{enumerate}
\end{theorem}
\end{frame}
...
\begin{frame}
\begin{corollary}
Let $\b0\leq A\in \M_n$ be irreducible with exactly $h$ distinct eigenvalues of modulus $\rho(A)$. Then these eigenvalues are the vertices of a regular polygon of $h$ sides with centre at the origin and are of the vertices being $\rho(A)$

\begin{tikzpicture}
\node  (I0) at (1.4,-0.3) {$\rho(A)$};
\node  (I0) at (1,0) {$\bullet$};
...
\begin{frame}
\begin{theorem}
Let $\b0\leq A\in\M_n$, $n \geq 2$. TFAE
\begin{enumerate}
    \item $A^n=\b0$
    \item There exists $\IN\ni k>0$ such that $A^k=\b0$
    \item $G(A)$ is acyclic
    \item $\exists P$, permutation matrix, s.t. $PAP^T$ is upper-triangular with zeros on main diagonal
    \item $\rho(A)=0$
\end{enumerate}
\end{theorem}
\vfill
\begin{theorem}
Let $\b0\leq A\in\M_n$. Assume that $A$ has a positive eigenvector. Then that eigenvector is the Perron vector and is associated to $\rho(A)$
\end{theorem}
\end{frame}
...
The matrix $A\in \M_n$ is \defword{row stochastic} (sometimes \defword{right stochastic})  if
\begin{enumerate}
    \item[-] $A\geq\b0$ \hfill[The matrix is nonnegative]
    \item[-] $A\nbOne = \nbOne$,  $\nbOne=(1,\ldots,1)^T$ \hfill[All rows sum to 1]
\end{enumerate}
\vskip0.5cm
The matrix $A\in \M_n$ is \defword{column stochastic} (sometimes \defword{left stochastic})  if
\begin{enumerate}
	\item[-] $A\geq\b0$ \hfill[The matrix is nonnegative]
	\item[-] $\nbOne^TA = \nbOne^T$ \hfill[All columns sum to 1]
\end{enumerate}
\vskip0.5cm
The matrix $A\in \M_n$ is \defword{stochastic} if it is either row or column stochastic. The matrix is \defword{doubly stochastic} if it is both row and column stochastic
\end{definition}
\end{frame}
...
\vfill
\begin{theorem}
Let $\b0\leq P\in\M_n$. Assume that $P$ has a positive eigenvector $\bu$ and that $\rho(P)>0$. Then there exists $D$, diagonal matrix with $\diag(D)>\b0$, and $0<k\in \IR$ such that 
\[
A=kDPD^{-1}
\]
...
\begin{frame}
\begin{definition}
The matrix $\b0\leq A\in \M_n$ is \defword{doubly stochastic}\index{matrix!doubly stochastic} if $A\nbOne =\nbOne$ and $\nbOne^T A= \nbOne^T$
\end{definition}
\vfill
\begin{remark}
